...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  switch (blocknr)
    ...
    case IO_NRHO:		/* density */
      for(n = 0; n < pc; pindex++)
	if(P[pindex].Type == type)
	  {
	    *fp++ = SphP[pindex].Density*All.UnitDensity_in_cgs/(1.44*PROTONMASS);
	    n++;
	  }
      break;
...
void fill_Tab_IO_Labels(void)
{
  ...
  for(i = 0; i < IO_NBLOCKS; i++)
	...
	break;
      case IO_TEMP:
	strncpy(Tab_IO_Labels[IO_U], "TEMP   ", 4);
	break;
      case IO_RHO:
